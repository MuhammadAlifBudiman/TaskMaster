...
class RegistrationLoginTest(TestCase):
    def setUp(self):
        """
        Create a valid user for testing registration and login.
        """
...
class TaskViewSetTestCase(APITestCase):
    ...
    @parameterized.expand([
        ...
        ('', '', '', '', '', '', '', '', '', {
            ...
            "execution_time": [
                ErrorDetail(string="Time has wrong format. Use one of these formats instead: hh:mm[:ss[.uuuuuu]].", code='invalid')
            ],
            "execution_date": [
        ...
        ('title', 'description', False, True, False, False, None, None, None, {
            "execution_time": [
                ErrorDetail(string='This field may not be blank.', code='invalid')
            ],
        }),
        # Test missing field for weeklytask
        ...
        ('title', 'description', False, False, True, False, 'invalid', '20:20', None, {
            "execution_day": [
                ErrorDetail(string='Invalid value. Choose from: Sunday, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday.', code='invalid')
            ],
        }),
        # Test invalid execution date
        ...
        ('title', 'description', False, False, False, True, None, '20:20', 32, {
            "execution_date": [
                ErrorDetail(string='Ensure this value is less than or equal to 31.', code='max_value')
            ],
        }),
        # Test invalid execution date
        ...
        ('title', 'description', False, True, False, False, 'Monday', '20:20', 31, {
            "dailytask": [
                ErrorDetail(string='Invalid field for daily task.', code='invalid')
            ],
        }),
        # Test wrong field in weeklytask
        ('title', 'description', False, False, True, False, 'Monday', '20:20', 31, {
            "weeklytask": [
                ErrorDetail(string='Invalid field for weekly task.', code='invalid')
            ],
        }),
        # Test wrong field in monthlytask
        ('title', 'description', False, False, False, True, 'Monday', '20:20', 31, {
            "monthlytask": [
                ErrorDetail(string='Invalid field for monthly task.', code='invalid')
            ],
        }),
        # Test invalid type
        ('title', 'description', False, True, True, True, 'Monday', '20:20', 31, {
            "type error": [
                ErrorDetail(string="Please select one and only one of 'Daily', 'Weekly', or 'Monthly'.", code='invalid')
            ],
        }),
        # Test invalid type
    ...
    @parameterized.expand([
        ...
        ('title', 'description', False, False, True, False, 'Monday', '20:20', None),
...
class ExportTaskToExcelTestCase(TestCase):
    ...
    def test_export_task_to_excel(self):
        ...
        self.assertEqual(len(workbook.sheetnames), 3)  # Assuming three task types: daily, weekly, monthly
        ...
        self.assertEqual(daily_sheet.cell(row=3, column=2).value, 'Daily Task 1')
        self.assertEqual(daily_sheet.cell(row=2, column=3).value, 'Description')
        self.assertEqual(daily_sheet.cell(row=3, column=3).value, 'This is a daily task')
        self.assertEqual(daily_sheet.cell(row=2, column=4).value, 'Execution_time')
        ...
        self.assertEqual(weekly_sheet.cell(row=3, column=1).value, '05-07-2023')
        self.assertEqual(weekly_sheet.cell(row=2, column=2).value, 'Title')
        self.assertEqual(weekly_sheet.cell(row=3, column=2).value, 'Weekly Task 1')
        self.assertEqual(weekly_sheet.cell(row=2, column=3).value, 'Description')
        self.assertEqual(weekly_sheet.cell(row=3, column=3).value, 'This is a weekly task')
        self.assertEqual(weekly_sheet.cell(row=2, column=4).value, 'Execution_time')
        self.assertEqual(weekly_sheet.cell(row=3, column=4).value, 'Wednesday, 08:00')
        ...
        self.assertEqual(monthly_sheet.cell(row=3, column=1).value, '05-07-2023')
        self.assertEqual(monthly_sheet.cell(row=2, column=2).value, 'Title')
        self.assertEqual(monthly_sheet.cell(row=3, column=2).value, 'Monthly Task 1')
        self.assertEqual(monthly_sheet.cell(row=2, column=3).value, 'Description')
        self.assertEqual(monthly_sheet.cell(row=3, column=3).value, 'This is a monthly task')
        self.assertEqual(monthly_sheet.cell(row=2, column=4).value, 'Execution_time')
        self.assertEqual(monthly_sheet.cell(row=3, column=4).value, 'Day 31, 08:00')
        self.assertEqual(monthly_sheet.cell(row=2, column=5).value, 'Completed')
